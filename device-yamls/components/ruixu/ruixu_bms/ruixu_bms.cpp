...
static const char *const TAG = "ruixu_bms";

void RuixuBms::on_ruixu_modbus_data(const std::vector<uint8_t> &data) {
...
void RuixuBms::on_telemetry_data_(const std::vector<uint8_t> &data) {
  auto ruixu_get_16bit = [&](size_t i) -> uint16_t {
  ...
  for (uint8_t i = 0; i < std::min((uint8_t) 6, temperature_sensors); i++) {
    float raw_temperature = (float) ruixu_get_16bit(offset + 1 + (i * 2));
  ...
  float current = (float) ((int16_t) ruixu_get_16bit(offset)) * 0.1f;
  this->publish_state_(this->current_sensor_, current);

  //   56     0x14 0xA0      Total battery voltage            5280 * 0.01f = 52.80          V
  float total_voltage = (float) ruixu_get_16bit(offset + 2) * 0.01f;
  ...
  this->publish_state_(this->residual_capacity_sensor_, (float) ruixu_get_16bit(offset + 4) * 0.01f);

  //   60     0x0A           Custom number                    10
  //   61     0x42 0x68      Battery capacity                 17000 * 0.01f = 170.00        Ah
  this->publish_state_(this->battery_capacity_sensor_, (float) ruixu_get_16bit(offset + 7) * 0.01f);

  //   67     0x00 0x46      Number of cycles                 70
  this->publish_state_(this->charging_cycles_sensor_, (float) ruixu_get_16bit(offset + 9));
  ...
  this->publish_state_(this->port_voltage_sensor_, (float) ruixu_get_16bit(offset + 17) * 0.01f);
...
void RuixuBms::dump_config() {
  ESP_LOGCONFIG(TAG, "RuixuBms:");
...
float RuixuBms::get_setup_priority() const {
  // After UART bus
  return setup_priority::BUS - 1.0f;
}

void RuixuBms::update() { this->send(0x42, this->pack_); }

void RuixuBms::publish_state_(binary_sensor::BinarySensor *binary_sensor, const bool &state) {
...
void RuixuBms::publish_state_(sensor::Sensor *sensor, float value) {
  if (sensor == nullptr)
    return;
