...
from . import CONF_RUIXU_BMS_ID, RuixuBms

DEPENDENCIES = ["ruixu_bms"]
...
CONFIG_SCHEMA = cv.Schema(
    {
        cv.GenerateID(CONF_RUIXU_BMS_ID): cv.use_id(RuixuBms),
    ...
    # create the cells
    { cv.Optional(i): CELL_VOLTAGE_SCHEMA for i in CELLS }
).extend(
    # create the temps
    { cv.Optional(i): TEMP_SCHEMA for i in TEMPERATURES }
)


async def to_code(config):
    hub = await cg.get_variable(config[CONF_RUIXU_BMS_ID])
