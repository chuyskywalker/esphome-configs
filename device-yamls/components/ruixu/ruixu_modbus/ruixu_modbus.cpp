#include "ruixu_modbus.h"
#include "esphome/core/log.h"
#include "esphome/core/helpers.h"

namespace esphome {
namespace ruixu_modbus {

static const char *const TAG = "ruixu_modbus";

static const uint16_t MAX_RESPONSE_SIZE = 340;

void RuixuModbus::setup() {
  if (this->flow_control_pin_ != nullptr) {
    this->flow_control_pin_->setup();
  }
}
void RuixuModbus::loop() {
  const uint32_t now = millis();

  if (now - this->last_ruixu_modbus_byte_ > this->rx_timeout_) {
    ESP_LOGVV(TAG, "Buffer cleared due to timeout: %s",
              format_hex_pretty(&this->rx_buffer_.front(), this->rx_buffer_.size()).c_str());
    this->rx_buffer_.clear();
    this->last_ruixu_modbus_byte_ = now;
  }

  while (this->available()) {
    uint8_t byte;
    this->read_byte(&byte);
    if (this->parse_ruixu_modbus_byte_(byte)) {
      this->last_ruixu_modbus_byte_ = now;
    } else {
...
bool RuixuModbus::parse_ruixu_modbus_byte_(uint8_t byte) {
  ...
  for (auto *device : this->devices_) {
    if (device->address_ == address) {
      device->on_ruixu_modbus_data(data);
      found = true;
    }
  }

  if (!found) {
    ESP_LOGW(TAG, "Got RuixuModbus frame from unknown address 0x%02X! ", address);
  }
...
void RuixuModbus::dump_config() {
  ESP_LOGCONFIG(TAG, "RuixuModbus:");
  LOG_PIN("  Flow Control Pin: ", this->flow_control_pin_);
  ESP_LOGCONFIG(TAG, "  RX timeout: %d ms", this->rx_timeout_);
}
float RuixuModbus::get_setup_priority() const {
  // After UART bus
  return setup_priority::BUS - 1.0f;
}

void RuixuModbus::send(uint8_t protocol_version, uint8_t address, uint8_t function, uint8_t value) {
  if (this->flow_control_pin_ != nullptr)
